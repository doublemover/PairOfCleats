...
- 2026-03-02T11:33:49.0470629-05:00: Added solargraph workspace dependency-model preflight classification for missing `Gemfile.lock` (`solargraph_workspace_gemfile_lock_missing`) as fail-open degraded diagnostics with ci-lite coverage.
...
- 2026-03-02T13:55:41.1186186-05:00: Added configured gopls large-workspace warmup preflight probe path (`go_workspace_warmup_probe_*`) with bounded file-scan heuristics and explicit server overrides (`goWorkspaceWarmup*`) in normalization.
- 2026-03-02T13:55:41.1186186-05:00: Added pyright workspace scan outlier preflight telemetry/classification (`pyright_workspace_scan_outlier`) with configurable thresholds (`workspaceScanOutlierEntryThreshold`, `workspaceScanOutlierDurationMs`) and ci coverage.
- 2026-03-02T13:55:41.1186186-05:00: Added jdtls launch-script mismatch preflight classification (`jdtls_launch_script_mismatch`) for direct `java` command overrides missing required `-jar`/`-configuration` contract.
- 2026-03-02T13:55:41.1186186-05:00: Added csharp dotnet launch/bootstrap contract preflight classification (`csharp_launch_contract_invalid`, `csharp_launch_bootstrap_missing`) and dedicated ci-lite coverage.
- 2026-03-02T13:55:41.1186186-05:00: Added shared runtime probe helper and integrated elixir/solargraph runtime-aware preflight classification for OTP mismatch (`elixir_runtime_otp_mismatch`) and ruby toolchain deficiencies (`solargraph_runtime_toolchain_missing_*`).
- 2026-03-02T13:55:41.1186186-05:00: Strengthened dedicated-provider abrupt-failure cleanup test assertions from count-only checks to tracked subprocess lifecycle event closure checks (spawned process -> terminal event per pid).
