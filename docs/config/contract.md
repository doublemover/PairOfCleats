...
Env vars do not participate in normal behavior precedence, except the MCP server mode override (documented above) which takes effect only for `mcp-server` mode selection.
