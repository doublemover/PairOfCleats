...
## 8. Symbol identity integration (recommended in Phase 8)

Even if full SCIP/LSIF is not available, the TS provider can emit a heuristic `SymbolRef` that is consistent within a build and mostly stable across rebuilds.

### 8.1 Heuristic symbolKey from TS

For the bound node:
- derive symbol via `checker.getSymbolAtLocation(identifier)` (or `node.symbol`)
- derive a qualified name:
  - `checker.getFullyQualifiedName(symbol)` (string)
- set:

```
symbolKey = "sk:v1:" + sha1(namespaceKey + "|" + target.virtualPath + "|" + kind + "|" + fullyQualifiedName)
```

### 8.2 signatureKey

If a signature string is available:
- `signatureString = checker.signatureToString(sig, node, flags)`
- normalize and hash:
  - `signatureKey = "sig:v1:sha1:" + sha1(normalizedSignatureString)`

### 8.3 scopedId and symbolId fallback

- `scopedId = "sid:v1:sha1:" + sha1(symbolKey + "|" + (signatureKey||""))`
- `symbolId = "heur:ts:" + scopedId`

Attach this `SymbolRef` to the enrichment record for the chunkUid.
