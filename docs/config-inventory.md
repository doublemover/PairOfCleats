...
Generated: 2026-01-17T10:40:20.947Z
...
- PAIROFCLEATS_API_ALLOW_ANY_ORIGIN (1 files)
- PAIROFCLEATS_API_ALLOW_UNAUTHENTICATED (1 files)
- PAIROFCLEATS_API_ALLOWED_ORIGINS (1 files)
- PAIROFCLEATS_API_ALLOWED_REPO_ROOTS (1 files)
- PAIROFCLEATS_API_MAX_BODY_BYTES (1 files)
- PAIROFCLEATS_API_TOKEN (1 files)
...
- PAIROFCLEATS_MAX_OLD_SPACE_MB (5 files)
...
- PAIROFCLEATS_PROGRESS_FILES (1 files)
- PAIROFCLEATS_PROGRESS_LINES (1 files)
...
code-dir, compact, extracted-prose-dir, incremental, index-root, mode, out, progress, prose-dir, quiet, records-dir, repo, validate, verbose
...
ann, backend, dataset, out, pretty, progress, quiet, repo, top, verbose
...
```
api (object)
api.allowedRepoRoots (array)
api.auth (object)
api.auth.allowUnauthenticated (boolean)
api.cors (object)
api.cors.allowAnyOrigin (boolean)
api.cors.allowedOrigins (array)
api.maxBodyBytes (number)
cache (object)
cache.gc (object)
...
indexing.artifactCompression.mode (string) enum=auto|gzip|zstd|none
...
indexing.comments.includeInCode (boolean)
...
indexing.documentExtraction (object)
indexing.documentExtraction.enabled (boolean)
...
indexing.embeddings.lancedb (object)
indexing.embeddings.lancedb.batchSize (number)
indexing.embeddings.lancedb.embeddingColumn (string)
indexing.embeddings.lancedb.enabled (boolean)
indexing.embeddings.lancedb.idColumn (string)
indexing.embeddings.lancedb.metric (string) enum=cosine|l2|dot
indexing.embeddings.lancedb.table (string)
...
indexing.hash.backend (string) enum=auto|native|wasm
...
indexing.pythonAst.maxTextBytes (number)
...
indexing.watch (object)
indexing.watch.backend (string) enum=auto|chokidar|parcel
...
mcp.transport (string) enum=auto|sdk|legacy
models (object)
...
profile (string)
records (object)
records.detect (boolean)
records.excludeGlobs
records.includeGlobs
records.sniffBytes (number)
runtime (object)
runtime.maxOldSpaceMb (number)
runtime.nodeOptions (string)
runtime.uvThreadpoolSize (number)
search (object)
search.annBackend (string) enum=auto|lancedb|sqlite-vector|sqlite-extension|hnsw|js
...
search.regex (object)
search.regex.engine (string) enum=auto|re2|re2js
search.rrf (object)
