# Spec -- USR Gate Evaluation Algorithm
...
Defines deterministic hard-block/advisory gate computation for release readiness.
...
1. reject expired waivers
2. classify findings using gate policy mapping
3. promote warnings to blocking if escalation thresholds are exceeded
4. compute final gate state (PASS, ADVISORY_FAIL, BLOCKING_FAIL)
...
- warning findings in same reasonCode class recurring across 2 consecutive release windows MUST promote to blocking.
- stale blocking evidence (>24h) is blocking unless active unexpired waiver exists.
- any critical diagnostic code yields immediate blocking status.

## 5. Deterministic pseudocode

1. load findings, policy rows, waivers, freshness report
2. drop waivers where `now > allowedUntil`
3. map each finding to gate class and severity
4. apply escalation thresholds
5. compute final status and sorted blocking/advisory lists
