# USR Language Contract -- dockerfile
...
This document defines the dockerfile-specific USR contract as a child profile of:
...
- maxVersion: `null`
- dialects: `dockerfile`
- featureFlags: `multi-stage,buildkit`
...
dockerfile MUST define language-specific required mappings for declarations, callables, linkage constructs, and control/data constructs required by its conformance levels.
...
dockerfile MUST provide deterministic extraction for applicable edge kinds: defines, references, calls, contains, plus language-applicable imports, exports, and uses_type.
...
- multi-stage alias resolution; ARG and ENV propagation; COPY from linkage

## 6. Normalization mapping requirements

Raw parser kinds for dockerfile MUST map via table-driven mapping rules. Unknown kinds MUST preserve raw kind, map to normKind=unknown, and emit deterministic fallback diagnostics.
...
- dockerfile::risk::* (when C3 applies)

If this language hosts embedded surfaces, add:

- dockerfile::embedding-bridge::*
