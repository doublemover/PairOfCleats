...
This document defines the css-specific USR contract as a child profile of:
...
- Applicable framework overlays: react,next,vue,nuxt,svelte,sveltekit,angular,astro
...
- maxVersion: `null`
- dialects: `css`
- featureFlags: `nesting,container-queries`
...
css MUST define language-specific required mappings for declarations, callables, linkage constructs, and control/data constructs required by its conformance levels.

### 2.1 RequiredNodeKinds baseline

- css_rule, css_selector, directive_expr

## 3. Required edge coverage

css MUST provide deterministic extraction for applicable edge kinds: defines, references, calls, contains, plus language-applicable imports, exports, and uses_type.

### 3.1 RequiredEdgeKinds baseline

- contains, style_scopes, references
...
- scoping and module modes; nested selectors; at-rule dependency ordering

## 6. Normalization mapping requirements

Raw parser kinds for css MUST map via table-driven mapping rules. Unknown kinds MUST preserve raw kind, map to normKind=unknown, and emit deterministic fallback diagnostics.
...
- css::determinism::*
- css::provenance::*
- css::risk::* (when C3 applies)

If this language hosts embedded surfaces, add:

- css::embedding-bridge::*
