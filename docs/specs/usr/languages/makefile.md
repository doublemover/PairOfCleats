# USR Language Contract -- makefile
...
This document defines the makefile-specific USR contract as a child profile of:
...
- minVersion: `gnu-make-4`
- maxVersion: `null`
- dialects: `make`
- featureFlags: `pattern-rules,include`
...
makefile MUST define language-specific required mappings for declarations, callables, linkage constructs, and control/data constructs required by its conformance levels.

### 2.1 RequiredNodeKinds baseline

- build_stmt, variable_decl, call_expr

## 3. Required edge coverage

makefile MUST provide deterministic extraction for applicable edge kinds: defines, references, calls, contains, plus language-applicable imports, exports, and uses_type.
...
- target dependency graphs; variable expansion; shell command edge attribution

## 6. Normalization mapping requirements

Raw parser kinds for makefile MUST map via table-driven mapping rules. Unknown kinds MUST preserve raw kind, map to normKind=unknown, and emit deterministic fallback diagnostics.
...
- makefile::fallback::*
- makefile::resolution::*
- makefile::determinism::*
- makefile::provenance::*
- makefile::risk::* (when C3 applies)

If this language hosts embedded surfaces, add:

- makefile::embedding-bridge::*
