...
This document defines the swift-specific USR contract as a child profile of:
...
- maxVersion: `null`
- dialects: `swift`
- featureFlags: `actors,concurrency`
...
swift MUST define language-specific required mappings for declarations, callables, linkage constructs, and control/data constructs required by its conformance levels.
...
swift MUST provide deterministic extraction for applicable edge kinds: defines, references, calls, contains, plus language-applicable imports, exports, and uses_type.
...
- protocol extension dispatch; property wrappers; async and actor boundaries

## 6. Normalization mapping requirements

Raw parser kinds for swift MUST map via table-driven mapping rules. Unknown kinds MUST preserve raw kind, map to normKind=unknown, and emit deterministic fallback diagnostics.
...
- swift::determinism::*
- swift::provenance::*
- swift::risk::* (when C3 applies)

If this language hosts embedded surfaces, add:

- swift::embedding-bridge::*
