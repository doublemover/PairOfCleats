...
This document defines the typescript-specific USR contract as a child profile of:
...
typescript MUST define language-specific required mappings for declarations, callables, linkage constructs, and control/data constructs required by its conformance levels.
...
typescript MUST provide deterministic extraction for applicable edge kinds: defines, references, calls, contains, plus language-applicable imports, exports, and uses_type.
...
- type/value space separation; declaration merging; type-only imports and exports

## 6. Normalization mapping requirements

Raw parser kinds for typescript MUST map via table-driven mapping rules. Unknown kinds MUST preserve raw kind, map to normKind=unknown, and emit deterministic fallback diagnostics.
...
- typescript::risk::* (when C3 applies)

If this language hosts embedded surfaces, add:

- typescript::embedding-bridge::*
