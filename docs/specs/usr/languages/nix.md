...
This document defines the nix-specific USR contract as a child profile of:
...
nix MUST define language-specific required mappings for declarations, callables, linkage constructs, and control/data constructs required by its conformance levels.
...
nix MUST provide deterministic extraction for applicable edge kinds: defines, references, calls, contains, plus language-applicable imports, exports, and uses_type.
...
- attribute-set indirection; import call graphs; lazy evaluation references

## 6. Normalization mapping requirements

Raw parser kinds for nix MUST map via table-driven mapping rules. Unknown kinds MUST preserve raw kind, map to normKind=unknown, and emit deterministic fallback diagnostics.
...
- nix::determinism::*
- nix::provenance::*
- nix::risk::* (when C3 applies)

If this language hosts embedded surfaces, add:

- nix::embedding-bridge::*
