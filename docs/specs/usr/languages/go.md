...
This document defines the go-specific USR contract as a child profile of:
...
go MUST define language-specific required mappings for declarations, callables, linkage constructs, and control/data constructs required by its conformance levels.
...
go MUST provide deterministic extraction for applicable edge kinds: defines, references, calls, contains, plus language-applicable imports, exports, and uses_type.
...
- receiver method sets; interface embedding; goroutine and channel flow edges

## 6. Normalization mapping requirements

Raw parser kinds for go MUST map via table-driven mapping rules. Unknown kinds MUST preserve raw kind, map to normKind=unknown, and emit deterministic fallback diagnostics.
