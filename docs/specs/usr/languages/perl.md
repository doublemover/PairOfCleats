...
This document defines the perl-specific USR contract as a child profile of:
...
perl MUST define language-specific required mappings for declarations, callables, linkage constructs, and control/data constructs required by its conformance levels.
...
perl MUST provide deterministic extraction for applicable edge kinds: defines, references, calls, contains, plus language-applicable imports, exports, and uses_type.
...
- context-sensitive parsing; symbolic references; runtime eval and regex code execution

## 6. Normalization mapping requirements

Raw parser kinds for perl MUST map via table-driven mapping rules. Unknown kinds MUST preserve raw kind, map to normKind=unknown, and emit deterministic fallback diagnostics.
...
- perl::determinism::*
- perl::provenance::*
- perl::risk::* (when C3 applies)

If this language hosts embedded surfaces, add:

- perl::embedding-bridge::*
