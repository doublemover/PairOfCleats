...
This document defines the scala-specific USR contract as a child profile of:
...
- minVersion: `2.13`
- maxVersion: `null`
- dialects: `scala2,scala3`
- featureFlags: `givens,opaque-types`
...
scala MUST define language-specific required mappings for declarations, callables, linkage constructs, and control/data constructs required by its conformance levels.
...
scala MUST provide deterministic extraction for applicable edge kinds: defines, references, calls, contains, plus language-applicable imports, exports, and uses_type.
...
- implicit or given resolution; companion-object linking; for-comprehension lowering

## 6. Normalization mapping requirements

Raw parser kinds for scala MUST map via table-driven mapping rules. Unknown kinds MUST preserve raw kind, map to normKind=unknown, and emit deterministic fallback diagnostics.
...
- scala::determinism::*
- scala::provenance::*
- scala::risk::* (when C3 applies)

If this language hosts embedded surfaces, add:

- scala::embedding-bridge::*
