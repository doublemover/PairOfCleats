...
This document defines the sql-specific USR contract as a child profile of:
...
sql MUST define language-specific required mappings for declarations, callables, linkage constructs, and control/data constructs required by its conformance levels.

### 2.1 RequiredNodeKinds baseline

- sql_stmt, query_expr, literal_expr

## 3. Required edge coverage

sql MUST provide deterministic extraction for applicable edge kinds: defines, references, calls, contains, plus language-applicable imports, exports, and uses_type.
...
- dialect syntax; object qualification; dynamic SQL concatenation risk

## 6. Normalization mapping requirements

Raw parser kinds for sql MUST map via table-driven mapping rules. Unknown kinds MUST preserve raw kind, map to normKind=unknown, and emit deterministic fallback diagnostics.
...
- sql::determinism::*
- sql::provenance::*
- sql::risk::* (when C3 applies)

If this language hosts embedded surfaces, add:

- sql::embedding-bridge::*
