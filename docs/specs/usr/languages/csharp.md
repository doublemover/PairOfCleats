...
This document defines the csharp-specific USR contract as a child profile of:
...
- maxVersion: `null`
- dialects: `csharp`
- featureFlags: `nullable,top-level-statements`

## 1.2 Embedding policy baseline

- canHostEmbedded: `true`
- canBeEmbedded: `true`
- embeddedLanguageAllowlist: `html,css,javascript,razor`

## 2. Required syntax and node coverage

csharp MUST define language-specific required mappings for declarations, callables, linkage constructs, and control/data constructs required by its conformance levels.
...
csharp MUST provide deterministic extraction for applicable edge kinds: defines, references, calls, contains, plus language-applicable imports, exports, and uses_type.
...
- partial classes; extension methods; LINQ lowering

## 6. Normalization mapping requirements

Raw parser kinds for csharp MUST map via table-driven mapping rules. Unknown kinds MUST preserve raw kind, map to normKind=unknown, and emit deterministic fallback diagnostics.
...
- csharp::determinism::*
- csharp::provenance::*
- csharp::risk::* (when C3 applies)

If this language hosts embedded surfaces, add:

- csharp::embedding-bridge::*
