...
This document defines the julia-specific USR contract as a child profile of:
...
julia MUST define language-specific required mappings for declarations, callables, linkage constructs, and control/data constructs required by its conformance levels.
...
julia MUST provide deterministic extraction for applicable edge kinds: defines, references, calls, contains, plus language-applicable imports, exports, and uses_type.
...
- multiple dispatch resolution; macro expansion; module include linkage

## 6. Normalization mapping requirements

Raw parser kinds for julia MUST map via table-driven mapping rules. Unknown kinds MUST preserve raw kind, map to normKind=unknown, and emit deterministic fallback diagnostics.
...
- julia::determinism::*
- julia::provenance::*
- julia::risk::* (when C3 applies)

If this language hosts embedded surfaces, add:

- julia::embedding-bridge::*
