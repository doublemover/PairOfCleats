...
This document defines the dart-specific USR contract as a child profile of:
...
- maxVersion: `null`
- dialects: `dart`
- featureFlags: `null-safety,records`
...
dart MUST define language-specific required mappings for declarations, callables, linkage constructs, and control/data constructs required by its conformance levels.
...
dart MUST provide deterministic extraction for applicable edge kinds: defines, references, calls, contains, plus language-applicable imports, exports, and uses_type.
...
- library and part directives; extension methods; null-safe flow analysis

## 6. Normalization mapping requirements

Raw parser kinds for dart MUST map via table-driven mapping rules. Unknown kinds MUST preserve raw kind, map to normKind=unknown, and emit deterministic fallback diagnostics.
...
- dart::determinism::*
- dart::provenance::*
- dart::risk::* (when C3 applies)

If this language hosts embedded surfaces, add:

- dart::embedding-bridge::*
