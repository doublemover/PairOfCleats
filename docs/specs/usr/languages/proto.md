...
This document defines the proto-specific USR contract as a child profile of:
...
proto MUST define language-specific required mappings for declarations, callables, linkage constructs, and control/data constructs required by its conformance levels.

### 2.1 RequiredNodeKinds baseline

- interface_decl, type_alias_decl, field_decl

## 3. Required edge coverage

proto MUST provide deterministic extraction for applicable edge kinds: defines, references, calls, contains, plus language-applicable imports, exports, and uses_type.
...
- message/service/type linking; option resolution; import namespace conflicts

## 6. Normalization mapping requirements

Raw parser kinds for proto MUST map via table-driven mapping rules. Unknown kinds MUST preserve raw kind, map to normKind=unknown, and emit deterministic fallback diagnostics.
...
- proto::determinism::*
- proto::provenance::*
- proto::risk::* (when C3 applies)

If this language hosts embedded surfaces, add:

- proto::embedding-bridge::*
