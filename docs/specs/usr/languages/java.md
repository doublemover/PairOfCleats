...
This document defines the java-specific USR contract as a child profile of:
...
- maxVersion: `null`
- dialects: `java`
- featureFlags: `records,sealed-classes`
...
java MUST define language-specific required mappings for declarations, callables, linkage constructs, and control/data constructs required by its conformance levels.
...
java MUST provide deterministic extraction for applicable edge kinds: defines, references, calls, contains, plus language-applicable imports, exports, and uses_type.
...
- generic erasure impact on linking; annotations; nested class and member resolution

## 6. Normalization mapping requirements

Raw parser kinds for java MUST map via table-driven mapping rules. Unknown kinds MUST preserve raw kind, map to normKind=unknown, and emit deterministic fallback diagnostics.
...
- java::determinism::*
- java::provenance::*
- java::risk::* (when C3 applies)

If this language hosts embedded surfaces, add:

- java::embedding-bridge::*
