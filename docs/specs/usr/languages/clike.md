...
This document defines the clike-specific USR contract as a child profile of:
...
- minVersion: `c11/cpp17`
- maxVersion: `null`
- dialects: `c,cpp,objc`
- featureFlags: `preprocessor,macros,templates`
...
clike MUST define language-specific required mappings for declarations, callables, linkage constructs, and control/data constructs required by its conformance levels.

### 2.1 RequiredNodeKinds baseline

- module_decl, function_decl, type_alias_decl, variable_decl, call_expr

## 3. Required edge coverage

clike MUST provide deterministic extraction for applicable edge kinds: defines, references, calls, contains, plus language-applicable imports, exports, and uses_type.

### 3.1 RequiredEdgeKinds baseline

- imports, defines, references, calls, uses_type, contains
...
- preprocessor and macro boundaries; include resolution; template or operator overload ambiguity

## 6. Normalization mapping requirements

Raw parser kinds for clike MUST map via table-driven mapping rules. Unknown kinds MUST preserve raw kind, map to normKind=unknown, and emit deterministic fallback diagnostics.
...
- clike::determinism::*
- clike::provenance::*
- clike::risk::* (when C3 applies)

If this language hosts embedded surfaces, add:

- clike::embedding-bridge::*
