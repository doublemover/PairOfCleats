...
This document defines the lua-specific USR contract as a child profile of:
...
- maxVersion: `null`
- dialects: `lua`
- featureFlags: `metatables,luajit-compat`
...
lua MUST define language-specific required mappings for declarations, callables, linkage constructs, and control/data constructs required by its conformance levels.
...
lua MUST provide deterministic extraction for applicable edge kinds: defines, references, calls, contains, plus language-applicable imports, exports, and uses_type.
...
- table-based modules; metatable call paths; dynamic require resolution

## 6. Normalization mapping requirements

Raw parser kinds for lua MUST map via table-driven mapping rules. Unknown kinds MUST preserve raw kind, map to normKind=unknown, and emit deterministic fallback diagnostics.
