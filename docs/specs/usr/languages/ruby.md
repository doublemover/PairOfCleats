...
This document defines the ruby-specific USR contract as a child profile of:
...
- maxVersion: `null`
- dialects: `ruby`
- featureFlags: `pattern-matching,refinements`
...
ruby MUST define language-specific required mappings for declarations, callables, linkage constructs, and control/data constructs required by its conformance levels.
...
ruby MUST provide deterministic extraction for applicable edge kinds: defines, references, calls, contains, plus language-applicable imports, exports, and uses_type.
...
- open classes and modules; metaprogramming send and eval; dynamic constant lookup

## 6. Normalization mapping requirements

Raw parser kinds for ruby MUST map via table-driven mapping rules. Unknown kinds MUST preserve raw kind, map to normKind=unknown, and emit deterministic fallback diagnostics.
...
- ruby::determinism::*
- ruby::provenance::*
- ruby::risk::* (when C3 applies)

If this language hosts embedded surfaces, add:

- ruby::embedding-bridge::*
