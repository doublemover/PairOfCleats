...
This document defines the rust-specific USR contract as a child profile of:
...
- maxVersion: `null`
- dialects: `rust`
- featureFlags: `macros,proc-macros`
...
rust MUST define language-specific required mappings for declarations, callables, linkage constructs, and control/data constructs required by its conformance levels.
...
rust MUST provide deterministic extraction for applicable edge kinds: defines, references, calls, contains, plus language-applicable imports, exports, and uses_type.
...
- macro expansion boundaries; trait impl resolution; borrow and lifetime influenced flow

## 6. Normalization mapping requirements

Raw parser kinds for rust MUST map via table-driven mapping rules. Unknown kinds MUST preserve raw kind, map to normKind=unknown, and emit deterministic fallback diagnostics.
...
- rust::determinism::*
- rust::provenance::*
- rust::risk::* (when C3 applies)

If this language hosts embedded surfaces, add:

- rust::embedding-bridge::*
