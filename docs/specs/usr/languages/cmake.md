...
This document defines the cmake-specific USR contract as a child profile of:
...
- maxVersion: `null`
- dialects: `cmake`
- featureFlags: `generator-expressions,presets`
...
cmake MUST define language-specific required mappings for declarations, callables, linkage constructs, and control/data constructs required by its conformance levels.
...
cmake MUST provide deterministic extraction for applicable edge kinds: defines, references, calls, contains, plus language-applicable imports, exports, and uses_type.
...
- variable indirection; include and target linkage; generator expression handling

## 6. Normalization mapping requirements

Raw parser kinds for cmake MUST map via table-driven mapping rules. Unknown kinds MUST preserve raw kind, map to normKind=unknown, and emit deterministic fallback diagnostics.
...
- cmake::determinism::*
- cmake::provenance::*
- cmake::risk::* (when C3 applies)

If this language hosts embedded surfaces, add:

- cmake::embedding-bridge::*
