# USR Language Contract -- starlark
...
This document defines the starlark-specific USR contract as a child profile of:
...
- maxVersion: `null`
- dialects: `starlark`
- featureFlags: `bzl-load,rule-macros`
...
starlark MUST define language-specific required mappings for declarations, callables, linkage constructs, and control/data constructs required by its conformance levels.
...
starlark MUST provide deterministic extraction for applicable edge kinds: defines, references, calls, contains, plus language-applicable imports, exports, and uses_type.
...
- load visibility; rule macro expansion; repository and workspace boundary mapping

## 6. Normalization mapping requirements

Raw parser kinds for starlark MUST map via table-driven mapping rules. Unknown kinds MUST preserve raw kind, map to normKind=unknown, and emit deterministic fallback diagnostics.
...
- starlark::fallback::*
- starlark::resolution::*
- starlark::determinism::*
- starlark::provenance::*
- starlark::risk::* (when C3 applies)

If this language hosts embedded surfaces, add:

- starlark::embedding-bridge::*
