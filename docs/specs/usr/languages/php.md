...
This document defines the php-specific USR contract as a child profile of:
...
php MUST define language-specific required mappings for declarations, callables, linkage constructs, and control/data constructs required by its conformance levels.
...
php MUST provide deterministic extraction for applicable edge kinds: defines, references, calls, contains, plus language-applicable imports, exports, and uses_type.
...
- namespace aliasing; magic methods; template embedded PHP blocks

## 6. Normalization mapping requirements

Raw parser kinds for php MUST map via table-driven mapping rules. Unknown kinds MUST preserve raw kind, map to normKind=unknown, and emit deterministic fallback diagnostics.
...
- php::determinism::*
- php::provenance::*
- php::risk::* (when C3 applies)

If this language hosts embedded surfaces, add:

- php::embedding-bridge::*
