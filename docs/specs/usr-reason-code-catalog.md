...
Defines normalized reason codes used by diagnostics and gate reports.
...
- usr.link.unresolved-import
- usr.schema.unknown-key
- usr.quality.threshold-breach
- usr.ops.drill-missing

## 2. Required catalog columns
...
## 3. Baseline reason-code rows

| reasonCode | triggerCondition | requiredFields | remediationClass |
| --- | --- | --- | --- |
| `usr.schema.unknown-key` | strict validator sees undeclared key | file, keyPath, schemaVersion | schema-fix |
| `usr.link.unresolved-import` | import resolution exhausts candidates | sourceFile, importPath, candidates | resolution-fix |
| `usr.quality.threshold-breach` | quality metric violates policy row | metric, value, threshold, fixtureSetId | quality-regression |
| `usr.ops.drill-missing` | required drill artifact absent/stale | runbookId, artifactId, phase | operational-readiness |
| `usr.security.redaction-missing` | required redaction class not applied | classId, artifactId, fieldPath | security-hotfix |
