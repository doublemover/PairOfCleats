...
| `USR-E-SECURITY-GATE-FAILED` | Security gate evaluation produced blocking failure. | `phase=security` or `runtime`, `message` includes gate/check ID, `docUid` when scope is local. | Producer/gate evaluator MUST fail closed for blocking checks and record deterministic control evidence in reports. |
| `USR-E-SLO-BUDGET-FAILED` | Blocking SLO/benchmark/resource budget threshold was exceeded. | `phase=ops` or `runtime`, `message` includes lane/scope + metric threshold. | Gate evaluator MUST block promotion for blocking lanes and emit actionable budget breach context. |
...
| `USR-W-RESOLUTION-CANDIDATE-CAPPED` | Candidate set exceeded configured cap and was deterministically truncated. | `phase=relate`, `docUid` or `segmentUid`, `message` includes configured cap and retained count. | Producer MUST keep deterministic candidate ordering, record cap metadata, and keep reference non-resolved when ambiguity remains. |
...
| `USR-R-PARSER-TIMEOUT` | Parser timed out before producing deterministic payload needed for resolution. | `status=unresolved` or `ambiguous`, `resolver`, context includes parser identity where available. | Emit deterministic fallback/degradation metadata and schedule rerun with parser/runtime diagnostics. |
| `USR-R-PARSER-UNAVAILABLE` | Preferred parser/compiler source is unavailable at runtime. | `status=unresolved`, `resolver`, optional `targetName`. | Emit deterministic fallback chain selection metadata and capability downgrade when required. |
...
| `USR-R-RESOLUTION-CONFLICT` | Deterministic mapping/resolution candidates conflict with contract constraints and cannot converge. | `status=ambiguous` or `unresolved`, `resolver`, candidate and/or conflict context. | Preserve deterministic conflict evidence and emit schema/normalization remediation diagnostics. |
| `USR-R-REDACTION-REQUIRED` | Evidence payload contains sensitive fields requiring redaction before publish. | `status=unresolved` for publish-ready state, `resolver`/pipeline stage context. | Block publish path until redaction rules are applied and validated. |
| `USR-R-CANDIDATE-CAP-EXCEEDED` | Candidate list exceeded configured cap and required deterministic truncation. | `status=ambiguous`, `candidates` non-empty, cap metadata present. | Preserve bounded candidate set with cap metadata and keep unresolved/ambiguous state. |
| `USR-R-RESOURCE-BUDGET-EXCEEDED` | Runtime resource budget threshold exceeded for required resolution analysis. | `status=unresolved` or `ambiguous`, scope + budget key context. | Emit ops/performance diagnostics and retry under compliant resource budgets. |
| `USR-R-SECURITY-GATE-BLOCKED` | Security policy denied operation or output in strict mode. | `status=unresolved` for blocked output/ref, gate/control identifier in context. | Fail closed for blocked scope and emit deterministic control evidence. |
| `USR-R-SERIALIZATION-INVALID` | Serialization output failed canonical/schema validity checks. | `status=unresolved` for persisted artifact state, serializer identity context. | Re-emit canonical payload or fail write path in strict mode. |
...
| `graph-integrity` | `USR-E-EDGE-ENDPOINT-INVALID`, `USR-W-REFERENCE-AMBIGUOUS`, `USR-W-RESOLUTION-CANDIDATE-CAPPED`, `USR-W-HEURISTIC-BINDING` | blocking when conformance target requires resolved relations | indexing/graph owner |
...
| `ops-security-gates` | `USR-E-SECURITY-GATE-FAILED`, `USR-E-SLO-BUDGET-FAILED` | blocking in strict and promotion lanes | security + operations owner |
