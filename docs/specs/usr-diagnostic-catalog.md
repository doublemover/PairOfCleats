...
Defines canonical diagnostic IDs, severity classes, dedupe keys, and lifecycle behavior.

## 1. Diagnostic code format

USR-<DOMAIN>-<CLASS>-<NNN>
...
- critical: always hard-block in strict mode
- error: hard-block when mapped to blocking gate row
- warning: advisory unless escalation threshold exceeded
- info: non-gating telemetry

## 4. Baseline taxonomy table

| Code | Domain | Severity | Trigger |
| --- | --- | --- | --- |
| `USR-SCHEMA-UNKNOWN-001` | schema | error | unknown key in strict validator |
| `USR-LINK-UNRESOLVED-001` | linking | warning | unresolved import/reference with deterministic candidates exhausted |
| `USR-QUALITY-THRESHOLD-001` | quality | error | blocking threshold violated |
| `USR-OPS-DRILL-MISSING-001` | operations | critical | blocking drill evidence missing |
| `USR-SEC-REDACTION-001` | security | critical | required redaction class missing |
