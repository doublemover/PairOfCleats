# Spec -- USR Template Expression Binding Contract
...
Defines normalized semantics for template expressions and bindings.
...
1. Define canonical node and edge mapping for directives events and interpolation forms.
2. Define slot children context propagation semantics across framework profiles.
3. Define fallback behavior and diagnostics for unsupported template constructs.
...
- usr-template-expression-binding-contract-validation.json
- usr-template-expression-binding-contract-drift-report.json
