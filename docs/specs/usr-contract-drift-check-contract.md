# Spec -- USR Contract Drift Check Contract
...
Defines deterministic drift checks between umbrella spec, decomposed contracts, roadmap traceability, and matrix/evidence references.

## 1. Drift dimensions

- missing reference drift
- incompatible schema-key drift
- gate-classification drift
- evidence requirement drift

## 2. Deterministic algorithm

- collect contract inventory from docs/specs/usr-*.md
- collect matrix inventory from tests/lang/matrix/usr-*.json
- compare reference targets and semantic key sets
- emit sorted findings with stable IDs

## 3. Blocking policy

- missing blocking contract reference is blocking drift
- mismatch in schema-key semantics is blocking drift
- stale traceability entry without contract file is advisory first window then blocking

## 4. Evidence outputs

- usr-contract-drift-report.json
- usr-contract-drift-trend.json
