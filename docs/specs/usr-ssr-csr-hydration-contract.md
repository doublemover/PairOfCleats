# Spec -- USR SSR CSR Hydration Contract
...
Defines server client hydration boundary semantics and diagnostics.
...
1. Define canonical representation for server-only client-only and shared boundaries.
2. Define hydration mismatch classes and evidence requirements.
3. Define cross-surface boundary fallback and degradation policy.
...
- usr-ssr-csr-hydration-contract-validation.json
- usr-ssr-csr-hydration-contract-drift-report.json
