...
Status guidelines:
- `ok`: interprocedural ran (summaries computed), even if `summaryOnly=true`.
- `disabled`: interprocedural disabled via config or runtime gating.
- `timed_out`: propagation exceeded `caps.maxMs`.
