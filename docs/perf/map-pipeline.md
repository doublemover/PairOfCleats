...
4. Build file nodes and edges (imports, exports, calls, usages, dataflow, aliases).
5. Apply scope filters and collapse rules; enforce limits and optional top-K by degree.
6. Sort nodes and edges deterministically; assemble the map model and viewer settings.
...
- The pipeline is in-memory; large repos should rely on `maxFiles`, `maxMembersPerFile`,
  `maxEdges`, and `topKByDegree` to cap output size.
