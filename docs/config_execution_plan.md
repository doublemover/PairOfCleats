...
This document is an **execution-ready** plan: phased checklist, PR slices, file-level targets, and explicit exit criteria. It assumes the end-state described in `pairofcleats_config_hard_cut.md` (minimal config, minimal flags, secrets-only env, AutoPolicy-driven behavior).
...
- [ ] Remove profile references in docs (`docs/commands.md` currently states “Experimental commands require profile=full”).
...
- [ ] Update `docs/search.md` / `docs/search-contract.md` to reflect the new mechanism.
...
If `docs/query-cache.md` exists as user-configurable, collapse to:
...
- [ ] Delete:
  ...
  - docs: `docs/external-backends.md` references
...
- `docs/env-overrides.md` (rewrite to secrets-only)
- `docs/external-backends.md` (rewrite or delete)
- any “profile=full required” sections in `docs/commands.md`
