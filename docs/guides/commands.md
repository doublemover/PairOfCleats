...
## CLI entrypoints
- `pairofcleats service api` - Run local HTTP JSON API service.
- `pairofcleats service indexer` - Run indexer service queue/worker.
- `pairofcleats ingest ctags` - Ingest ctags JSONL symbols into normalized records.
- `pairofcleats ingest gtags` - Ingest GNU Global symbols into normalized records.
- `pairofcleats ingest lsif` - Ingest LSIF dumps into normalized records.
- `pairofcleats ingest scip` - Ingest SCIP indexes into normalized records.
