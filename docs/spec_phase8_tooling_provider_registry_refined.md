...
- **Identity + Symbol contracts**: `spec_phase8_identity_and_symbol_contracts_refined.md`  
...
export type ToolingVirtualDocument = {
  ...
  segmentId?: string | null;    // null if not segmented
...
export type ToolingTarget = {
  chunk: ChunkRef; // docId + chunkUid + chunkId + file + segmentId + optional range
