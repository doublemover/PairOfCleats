...
1. **[P3] Query-plan cache silently disables when both cache directories are unset**  
   ...
   Fix: enforce/derive a fallback cache path (or emit a hard warning/error when cache is enabled but no path exists).
...
2. **[P3] Query-result disk cache silently disables when both cache directories are unset**  
   ...
   Fix: require a resolved cache path when query caching is enabled, or define an explicit fallback path.
...
3. **[P3] Memory-first hot-cache path can be effectively disabled when cachePath is falsy**  
   ...
   Fix: use a stable synthetic key when no disk cache path exists.
...
4. **[P3] Empty-string cache dir behaves like "no cache"**  
   ...
   Fix: normalize empty-string cache dirs to a concrete fallback path or reject explicitly.
...
5. **[P3] Compatibility-key checks are serialized across modes**  
   ...
   Fix: run compatibility target probes in parallel with `Promise.all`.
...
6. **[P3] Tantivy attach/build path runs serially by mode**  
   ...
   Fix: parallelize independent attach operations.
...
7. **[P3] File-relations/repo-map mode loads are serialized**  
   ...
   Fix: overlap independent artifact loads with bounded parallelism.
...
8. **[P3] Optional extracted-prose disablement can be silent when output is suppressed**  
   ...
   Fix: always emit telemetry/status metadata even when `emitOutput=false`.
...
100. **[P3] POSIX path normalization collapses implementation-defined leading `//` roots**  
     Evidence: `src/shared/path-normalize.js`  
...
101. **[P2] Atomic write fallback temp directory may not exist**  
...
102. **[P3] `atomicWriteText` newline option is ignored for Buffer payloads**  
...
103. **[P2] Cache-root resolver double-appends `cache-v1` for already-versioned overrides**  
...
104. **[P2] Cache clear operation targets wrong directory for already-versioned overrides**  
     Evidence: `src/shared/cache-roots.js`  
...
105. **[P2] Directory containment helper is symlink-blind**  
...
106. **[P2] Manifest loader errors are frequently untyped (`err.code` missing)**  
...
107. **[P3] Artifact presence resolver ignores legacy fallback paths**  
...
108. **[P3] Non-strict array/object loaders still fail on multi-entry manifest ambiguity**  
...
109. **[P2] Packed token-postings load path lacks checksum validation parity**  
...
110. **[P3] `existsOrBak` positive checks can still fail reads when only `.bak` exists**  
...
111. **[P3] Manifest cache freshness key is weak (size + mtime only)**  
...
112. **[P1] Per-file artifact metadata paths are not constrained to index root**  
...
113. **[P2] Chunk-meta loader can prefer stale binary-columnar artifacts not declared by manifest**  
...
114. **[P2] Optional index artifact loaders swallow corruption errors even in strict flows**  
...
115. **[P3] Optional-row loader fallback detection depends on message text prefix**  
     Evidence: `src/retrieval/cli-index.js`  
...
116. **[P2] Scheduler low-resource mode bypasses scheduling caps entirely**  
...
117. **[P2] JSONL iterator can deadlock on early consumer cancellation with bounded in-flight queue**  
...
118. **[P3] TOML import collector strips inline comments inside quoted values**  
     Evidence: `src/index/language-registry/import-collectors/toml.js`  
...
119. **[P3] INI import collector strips comment markers inside quoted values**  
     Evidence: `src/index/language-registry/import-collectors/ini.js`  
...
120. **[P3] YAML import collector strips `#` from quoted scalar values**  
     Evidence: `src/index/language-registry/import-collectors/yaml.js`  
...
121. **[P3] Changed-only coverage filtering omits untracked files**  
...
122. **[P2] Malformed V8 coverage JSON can abort entire coverage aggregation**  
     Evidence: `tools/testing/coverage/index.js`  
...
123. **[P3] Coverage merge summation over-counts total ranges across artifacts**  
     Evidence: `tools/testing/coverage/index.js`  
...
124. **[P3] Last-failure utility does not scan the active test log root**  
...
125. **[P2] Phase-22 gate script resolves test paths from invocation CWD, not repo root**  
...
126. **[P2] API test helper `requestRaw` still force-parses JSON**  
...
127. **[P3] Last-failure utility ignores `.testLogs/latest` fast path**  
