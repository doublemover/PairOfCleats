  ...
  const handleRequest = async (req, res) => {
    ...
    if (req.method === 'GET') {
      if (pathname.startsWith('/three/examples/')) {
        ...
        res.writeHead(200, { 'Content-Type': contentTypeFor(targetPath), 'Access-Control-Allow-Origin': '*' });
    ...
    if (pathname === '/map' && req.method === 'GET') {
      ...
      try {
        ...
        sendError(res, 500, ERROR_CODES.INTERNAL, err?.message || 'Failed to build map.');
        return;
      }
    ...
    if (pathname === '/map/nodes' && req.method === 'GET') {
      ...
      try {
        ...
        sendError(res, 500, ERROR_CODES.INTERNAL, err?.message || 'Failed to build map nodes.');
        return;
      }
