...
const main = async () => {
  ...
  const nodeKindMappingJson = await readJsonFromRoot(config.inputs.nodeKindMapping);
  const edgeKindConstraintsJson = await readJsonFromRoot(config.inputs.edgeKindConstraints);
  ...
  finishGate({
    report,
    passedMessage: 'item 39 gate passed',
    failedMessage: 'item 39 gate failed',
    strict: argv.strict
  });
};
