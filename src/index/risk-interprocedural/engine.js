...
export const computeInterproceduralRisk = ({
  ...
  if (!enabled || summaryOnly) {
    ...
    stats.reason = !enabled ? 'disabled' : 'summaryOnly';
  ...
  const maxTotalFlows = Number.isFinite(caps.maxTotalFlows) ? Math.max(0, caps.maxTotalFlows) : 0;
  ...
  while (queueIndex < queue.length) {
    ...
    if (sinkSignals.length && state.chunkUid !== state.rootSource.chunkUid) {
      for (const sink of sinkSignals) {
        ...
        if (maxTotalFlows && flowRows.length >= maxTotalFlows) {
