   ...
   */
    const runStateWriteProbe = async ({
      ...
      return runWithHangProbe({
        ...
        meta: safeMeta,
        run: async () => {
          // Keep warning timer referenced while awaiting state write so the
          // awaited path cannot evaporate under an empty event loop.
   ...
   */
    const runStateWriteBestEffort = async ({
      ...
      const timeoutPromise = new Promise((resolve) => {
        // Keep timeout referenced so Promise.race deterministically resolves.
