...
import {
  coercePositiveInt,
  parseEffectiveMaxOldSpaceMb,
  parseNodeOptions,
  parseUvThreadpoolSize
} from './parse.js';
...
export function resolveRuntimeEnvelope(input = {}) {
  ...
  if (
    Number.isFinite(baseMaxOldSpace)
    && Number.isFinite(requestedMaxOldSpace.value)
  ...
  return {
    ...
    runtime: {
      ...
      ioOversubscribe: makeSourcedValue(
        !!requestedIoOversubscribe.value,
        requestedIoOversubscribe.source,
        requestedIoOversubscribe.detail
      )
    },
    concurrency: {
      cpuCount: resolvedCpuCount,
