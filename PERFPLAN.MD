...
- [x] Add a duplicate-utility scan and document required shared usage.
- [x] Document shared component expectations and examples.

### Tests
- [x] `tests/shared/provenance.test.js`
- [x] `tests/shared/truncation.test.js`
- [x] `tests/shared/limits-normalization.test.js`
- [x] `tests/perf/shared-component-audit.test.js`
...
- [x] Global embeddings cache reduces recomputation and passes tests.
- [x] Search CLI startup time improved without functional regressions.
- [x] VFS correctness/perf improvements land before tree-sitter load changes.
- [x] Retrieval, graph/context-pack, map, and shared IO perf improvements landed with tests.
- [x] No artifact schema regressions.
...
- [x] `tests/indexing/filter-index/filter-index-sharding.test.js`
...
- [x] Use `@mongodb-js/zstd` for compression.
...
- [x] `tests/indexing/embeddings/cache-identity-invalidation.test.js`
...
### Tasks
- [ ] **6.0 Phase summary**
- [ ] Task 6.0.1: Audit incremental vs artifacts build memory usage.
- [ ] Task 6.0.2: Optimize batch sizes and streaming inserts.
- [ ] Task 6.0.3: Ensure WAL checkpointing and temp file cleanup are bounded.
...
### Tasks
- [x] Build and reuse symbol/edge indexes instead of re-sorting per request.
- [x] Cache graph relations and reuse across calls within a session.
- [x] Use range reads for excerpts instead of full file loads.
- [x] Reduce intermediate array churn in neighborhood traversal.
...
### Acceptance
- [x] Search CLI startup time improves measurably on cold start.
...
### Tasks
- [ ] **12.0 Phase summary**
- [ ] Task 12.0.1: Stream map build output and avoid full graph materialization.
- [ ] Task 12.0.2: Precompute edge aggregates and reduce per-frame allocations.
- [ ] Task 12.0.3: Use instancing + frustum culling for large node sets.
- [ ] Task 12.0.4: Add LOD strategy for edges/labels under load.
- [ ] Task 12.0.5: Add viewer telemetry for dropped frames and memory usage.

- [x] **12.A Map build streaming + memory bounds**
...
- [x] `tests/map/map-viewer-instancing-count.test.js`
  - Success: instanced meshes are used for files and members.
- [x] `tests/map/map-viewer-culling-correctness.test.js`
  - Success: culling hides/restores instanced buckets correctly.
- [x] `tests/map/map-viewer-lod-switch.test.js`
  - Success: LOD tier changes based on zoom + counts.
- [x] `tests/map/map-viewer-selection-stability.test.js`
  - Success: selected buckets remain visible under culling.
- [x] `tests/map/map-viewer-telemetry.test.js`
  - Success: FPS/dropped frame counters advance per frame.
...
### Tests
- [x] `tests/docs/embeddings-cache-docs-sync.test.js`
...
- [ ] Task 14.B.9: Add append‑only delta records for build_state writes with periodic compaction.
- [ ] Task 14.B.9.a: Write `{op, path, value, ts}` deltas to `build_state.deltas.jsonl`.
- [ ] Task 14.B.9.b: Add compaction job to rewrite a full snapshot when delta log exceeds a size threshold.
- [ ] Task 14.B.9.c: Ensure compaction is atomic and never blocks primary writes.
- [ ] Task 14.B.10: Add write‑through cache for serialized checkpoint blocks to avoid re‑stringify.
- [ ] Task 14.B.10.a: Cache JSON strings per section keyed by snapshot hash + size.
- [ ] Task 14.B.10.b: Invalidate on dirty flags or schema/version changes.
...
- [ ] Task 14.C.7: Add a path‑segment trie cache for rapid “exists” prefix checks.
- [ ] Task 14.C.7.a: Reuse trie to short‑circuit repeated path joins under common roots.
- [ ] Task 14.C.8: Add negative cache TTL so new files are eventually re‑checked.
- [ ] Task 14.C.8.a: Evict negative entries after a short window (e.g. 30–60s).
- [ ] Task 14.C.9: Precompile `paths`/`baseUrl` matchers into regexes for faster specifier resolution.
- [ ] Task 14.C.9.a: Cache compiled matchers per tsconfig fingerprint.
...
- [ ] Task 14.D.3: Preserve canonical docId ordering even when processing order changes.
- [ ] Task 14.D.3.a: Add `canonicalOrderIndex` from discovery order and keep processing reorder separate.
- [ ] Task 14.D.3.b: Ensure `orderedAppender` uses canonical order for docId assignment.
- [ ] Task 14.D.3.c: Add optional windowed batching to cap pending buffers when processing order diverges.
- [ ] Task 14.D.3.d: Detect out‑of‑order chunk_meta stream; only enable external sort when needed.
- [ ] Task 14.D.3.e: External sort by numeric `id` only to preserve docId alignment.
- [ ] Task 14.D.3.f: Stream JSONL directly when already ordered (skip spill collector).
- [ ] Task 14.D.3.g: Emit chunk_meta ordering telemetry in stage checkpoints.
- [ ] Task 14.D.3.h: Add docId alignment guardrails and verification in tests.
- [ ] Task 14.D.3.i: Add bucketed external sort by `id` range to avoid full compare sort.
- [ ] Task 14.D.3.j: Log the first out‑of‑order pair (prevId/nextId) for diagnostics.
- [ ] Task 14.D.3.k: Add ID‑only spill index to re‑order without re‑serializing rows.
- [ ] Task 14.D.3.l: Cache JSONL row strings when measuring to avoid double stringify in sorting paths.
- [ ] Touchpoints 14.D.3: `src/index/build/indexer/steps/discover.js`, `src/index/build/indexer/steps/process-files/tree-sitter.js`, `src/index/build/indexer/steps/process-files/ordered.js`, `src/index/build/artifacts/writers/chunk-meta.js`, `src/index/build/artifacts/helpers.js`, `src/index/validate/checks.js`.
...
- [ ] Task 14.D.6: Add optional columnar chunk_meta format with boundary conversion to row objects.
- [ ] Task 14.D.6.a: Keep compatibility loader that inflates columnar arrays to row objects on demand.
- [ ] Task 14.D.6.b: Add config switch + manifest flag for columnar chunk_meta.
...
- [ ] Task 14.E.7: Add optional columnar JSONL format for symbol occurrences/edges.
- [ ] Task 14.E.7.a: Convert to row objects at boundaries only (loaders + CLI).
- [ ] Task 14.E.8: Add canonical string tables for symbol names/roles/kinds to reduce repetition.
- [ ] Task 14.E.8.a: Store string tables in meta and reference by index in rows.

- [ ] **14.F Graph relations streaming + measurement**
...
- [ ] Task 14.F.5: Add CSR‑style adjacency format for graph relations (offsets + edge list).
- [ ] Task 14.F.5.a: Keep JSONL output for compatibility and emit CSR as optional artifact.
- [ ] Task 14.F.6: Add byte‑budgeted streaming caps for graph_relations to avoid runaway growth.
- [ ] Task 14.F.6.a: Record precise drop counts and cap thresholds per graph.

- [ ] **14.G Chargram postings memory bounds**
...
- [ ] Task 14.G.7: Add rolling hash/sliding window chargram generator to reduce allocations.
- [ ] Task 14.G.7.a: Implement a reusable buffer to avoid substring allocations per token.
- [ ] Task 14.G.8: Add df‑based cutoff for high‑frequency chargrams to bound postings growth.
- [ ] Task 14.G.8.a: Track dropped high‑df chargrams in stage checkpoints.

- [ ] **14.H Benchmarks + acceptance harness**
- [ ] Task 14.H.1: Implement `tools/bench/index/build-state-write.js` comparing baseline vs debounced writes.
- [ ] Task 14.H.1.a: Emit JSON summary with p50/p95 latency and write counts + delta.
- [ ] Task 14.H.2: Implement `tools/bench/index/import-resolution-graph.js` for nodes/edges throughput.
- [ ] Task 14.H.2.a: Report node/edge counts and cap triggers; include seed and baseline delta.
- [ ] Task 14.H.3: Implement `tools/bench/index/symbol-artifacts.js` to compare streaming vs baseline.
- [ ] Task 14.H.3.a: Include memory watermark and output hash compare.
...
- [ ] Task 14.H.6: Implement `tools/bench/index/chunk-meta-stream.js` for chunk_meta memory.
- [ ] Task 14.H.6.a: Include trim stats and output hash + baseline delta.
- [ ] Task 14.H.7: Document how to run benchmarks and expected deltas in `docs/perf/index-artifact-pipelines.md`.
- [ ] Task 14.H.8: Implement `tools/bench/index/jsonl-offset-index.js` for random-access lookups.
- [ ] Task 14.H.8.a: Compare baseline scan vs offset index (p50/p95 row fetch latency).
- [ ] Task 14.H.9: Implement `tools/bench/index/postings-packed.js` to compare JSON vs packed postings.
- [ ] Task 14.H.9.a: Report size ratio + load time delta for token/phrase/chargram postings.
- [ ] Task 14.H.10: Implement `tools/bench/index/jsonl-compression-pipeline.js` for write throughput.
- [ ] Task 14.H.10.a: Compare sync write vs worker compression (wall time + CPU).
- [ ] Task 14.H.11: Implement `tools/bench/index/import-graph-incremental.js` for incremental rebuilds.
- [ ] Task 14.H.11.a: Report reuse ratio (cached vs recomputed) + total time delta.

- [ ] **14.I Artifact access + write acceleration**
- [ ] Task 14.I.1: Add byte‑offset indexes for JSONL artifacts (chunk_meta, symbols, symbol_occurrences, symbol_edges, graph_relations, call_sites).
- [ ] Task 14.I.1.a: Emit offsets during JSONL write (no second pass); store `offsets.bin` (uint64 LE) + `offsets.meta.json`.
- [ ] Task 14.I.1.b: For compressed shards, either keep raw JSONL alongside compressed or use seekable zstd frames; document fallback.
- [ ] Task 14.I.1.c: Add loader utilities for fast row fetch by index (single read + JSON.parse).
- [ ] Task 14.I.1.d: Keep offsets in monotonically increasing order; validate last offset + row length against file size.
- [ ] Task 14.I.2: Add per‑file symbol row indexes for `symbol_occurrences`/`symbol_edges`.
- [ ] Task 14.I.2.a: Resolve `fileId` while streaming (via host chunkUid -> chunk_meta -> fileId); accumulate row offsets per file.
- [ ] Task 14.I.2.b: Store per‑file offset lists in packed varint arrays with a small JSON meta.
- [ ] Task 14.I.2.c: Add fast path in tooling to load per‑file rows without scanning all parts.
- [ ] Task 14.I.2.d: Keep per‑file offset lists sorted by row index to preserve deterministic output.
- [ ] Task 14.I.3: Add write batching + worker thread compression for JSONL artifacts.
- [ ] Task 14.I.3.a: Buffer rows into fixed-size byte blocks (e.g., 1–4MB) before writing.
- [ ] Task 14.I.3.b: Compress blocks in a bounded worker pool; preserve output ordering.
- [ ] Task 14.I.3.c: Backpressure when compression lags to avoid unbounded memory.
- [ ] Task 14.I.3.d: Prefer zstd (seekable frames) for large shards; fall back to gzip where required.
- [ ] Task 14.I.4: Add packed postings format (delta+varint) for docId lists.
- [ ] Task 14.I.4.a: Encode docId deltas + tf counts in a binary blob with offsets per token.
- [ ] Task 14.I.4.b: Keep JSON as fallback; loader prefers packed when present.
- [ ] Task 14.I.4.c: Use block-encoded varints (e.g., 128/256 doc blocks) to balance decode speed + compression.
- [ ] Task 14.I.5: Add incremental import‑graph refresh.
- [ ] Task 14.I.5.a: Persist per‑file resolution results keyed by file hash + tsconfig fingerprint.
- [ ] Task 14.I.5.b: Invalidate cache on tsconfig/package.json/paths changes; reuse for unchanged files.
- [ ] Task 14.I.5.c: Track reuse ratio + invalidations in stage checkpoints for tuning.
- [ ] Touchpoints 14.I: `src/index/build/artifacts/writers/*.js`, `src/shared/artifact-io/*.js`, `src/index/build/import-resolution-graph.js`, `src/index/build/indexer/steps/discover.js`, `src/index/build/indexer/steps/process-files/ordered.js`, `src/retrieval/*` loaders.
...
- [ ] `tests/indexing/imports/import-graph-incremental-reuse.test.js` (new)
...
- [ ] `tests/indexing/postings/postings-packed-roundtrip.test.js` (new)
