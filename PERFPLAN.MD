...
#### Tasks
- [ ] Add an in-memory query plan cache keyed by query + normalized config + index signature.
- [ ] Persist parse + plan shape (AST, tokens, phrase ngrams, filter hints) inside the plan.
- [ ] Define plan cache key material explicitly (normalized query, tokenizer version, parser version, retrieval config hash, index signature hash).
- [ ] Add a plan schema with versioning and validation.
- [ ] Implement cache eviction (size cap + TTL) and explicit invalidation hooks for config reload.
- [ ] Ensure plan cache invalidates on config changes (tokenization, filters, ANN backend, scoring).
- [ ] Ensure plan cache invalidates on index signature changes.
- [ ] Record cache hits/misses in metrics.

#### Tests
- [ ] `tests/retrieval/pipeline/query-cache-hit.test.js`
- [ ] `tests/retrieval/pipeline/query-plan-invalidates-config.test.js`
- [ ] `tests/retrieval/pipeline/query-plan-invalidates-index.test.js`
- [ ] `tests/retrieval/pipeline/query-plan-requirements.test.js`
- [ ] `tests/retrieval/pipeline/query-plan-cache-key.test.js`
- [ ] `tests/retrieval/pipeline/query-plan-eviction.test.js`
- [ ] `tests/retrieval/pipeline/query-plan-schema-version.test.js`

#### Acceptance
- [ ] Query parse and plan reuse across modes.
- [ ] Plan cache invalidates deterministically on config/index changes.
- [ ] Plan cache stays within size/TTL bounds and reports hit/miss metrics.
...
#### Tasks
- [ ] Add retrieval stage checkpoints (parse, filter, candidates, ANN, fusion, rank, output).
- [ ] Emit optional timing/memory summary in verbose mode.
- [ ] Delay file reads for summaries/context until needed.
- [ ] Expand output caches to reuse formatting work.
- [ ] Stream large JSON outputs to avoid building full arrays in memory.
- [ ] Add a “no-context” output path that skips file reads entirely when context is not requested.

#### Tests
- [ ] `tests/retrieval/pipeline/retrieval-stage-checkpoints.test.js`
- [ ] `tests/retrieval/output/output-cache-hit.test.js`
- [ ] `tests/retrieval/output/json-streaming.test.js` (new)
- [ ] `tests/retrieval/output/context-skips.test.js` (new)

#### Acceptance
- [ ] Retrieval stage telemetry is recorded.
- [ ] Output path avoids unnecessary IO.
...
#### Tasks
- [ ] Introduce a query-plan “requiredArtifacts” set (filterIndex, fileRelations, graphRelations, repoMap, contextIndex, ann).
- [ ] Define artifact dependency rules (e.g., repoMap requires graphRelations, filterIndex requires tokenIndex) and enforce them.
- [ ] Load only required artifacts for the plan; defer everything else.
- [ ] Ensure lazy index loader only parses manifest/metadata first.
- [ ] Lazy-import ANN backends and storage adapters only when required by the plan.
- [ ] Parallelize required artifact loads with a small concurrency limit and deterministic ordering.
- [ ] Add search startup profiler checkpoints and document them.
- [ ] Add fast paths for `search --help`/`--version` without initializing retrieval internals.

#### Tests
- [ ] `tests/retrieval/pipeline/search-startup-fastpath.test.js`
...
#### Acceptance
- [ ] Search startup avoids loading unused artifacts.
- [ ] `search --help` avoids heavy initialization.
- [ ] Artifact gating respects dependency rules with deterministic load order.
...
#### Tasks
- [ ] Build bitmap representations for filter results and candidate sets.
- [ ] Use bitmap intersection/union for fast allowlist computation.
- [ ] Define a Set↔bitmap threshold (cardinality-based) and keep it configurable.
- [ ] Ensure bitmap↔id mapping remains stable across index versions.
- [ ] Compile and cache path/ext/pattern predicates in the query plan.
- [ ] Add early exit when allowlist is empty or unchanged.
- [ ] Keep `Set` fallback for small populations.

#### Tests
- [ ] `tests/retrieval/pipeline/filter-bitmap-shortcircuit.test.js`
- [ ] `tests/retrieval/pipeline/filter-bitmap-equivalence.test.js`
- [ ] `tests/retrieval/pipeline/filter-compiled-reuse.test.js` (new)
- [ ] `tests/retrieval/pipeline/filter-bitmap-threshold.test.js` (new)

#### Acceptance
- [ ] Filtered queries avoid large `Set` allocations.
- [ ] Filter semantics remain identical.
...
#### Tasks
- [ ] Add a `topk` helper with heap selection and stable tie‑breaking (score, id, secondary key).
- [ ] Define the exact tie‑break order (primary score desc, secondary normalized id asc, tertiary source rank).
- [ ] Document ordering invariants in `docs/perf/retrieval-pipeline.md` with examples.
- [ ] Add a `score-buffer` pool for candidate scores and ids; reuse across pipeline stages.
- [ ] Size the pool based on `k` and the max candidate fanout; cap to avoid unbounded growth.
- [ ] Stream candidate generation into top‑K rather than materializing full arrays.
- [ ] Implement a streaming reducer that only keeps `k + slack` best candidates.
- [ ] Add an early cutoff mechanism (score thresholds, `k` saturation, optional time budget).
- [ ] Define cutoff heuristics and ensure they never drop a potentially better candidate.
- [ ] Reduce duplicate candidate materialization between BM25 + ANN by using shared buffers.
- [ ] Merge score buffers in-place to avoid re-allocating intermediate arrays.
- [ ] Ensure ANN backends are only initialized when vector mode is active and required.
- [ ] Guard ANN initialization behind `requiredArtifacts` and vector mode flags.
- [ ] Implement a strict fallback path when ANN is unavailable (log + continue with sparse).
- [ ] Define a single warning log with clear reason and recovery path.
- [ ] Add metrics for candidate counts, buffer reuse, and cutoff triggers.
- [ ] Emit metrics in verbose mode and in perf checkpoints.
...
- [ ] `tests/retrieval/pipeline/topk-equivalence.test.js`
...
#### Acceptance
- [ ] Candidate and ranking stages reduce memory spikes and avoid full-array sorts on large inputs.
- [ ] Top‑K results match prior ordering within deterministic tie rules.
- [ ] ANN backends are only initialized when required and safe to use.
- [ ] Fallback to sparse-only ranking preserves correct results without hard failures.
...
#### Tasks
- [ ] Add lightweight persistent query-plan cache (optional disk cache) to avoid parse cost across runs.
- [ ] Use typed arrays for score accumulation and reduce object churn.
- [ ] Add small object pools for candidate structs and buffers to reduce GC churn.
- [ ] Add ANN backend preflight to avoid slow failures and repeated retries.
- [ ] Profile and cap expensive explain output work when not requested.

#### Tests
- [ ] `tests/retrieval/pipeline/query-plan-disk-cache.test.js` (new)
- [ ] `tests/retrieval/pipeline/ann-preflight.test.js` (new)
- [ ] `tests/retrieval/pipeline/query-plan-disk-cache-size-cap.test.js` (new)
- [ ] `tests/retrieval/pipeline/object-pool-leak.test.js` (new)

#### Acceptance
- [ ] Additional performance improvements measurable in microbenchmarks.
