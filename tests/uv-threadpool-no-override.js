...
try {
  ...
  throw new Error(
    `uv-threadpool-no-override test failed: invalid JSON output: ${err?.message || err}`
  );
}
...
if (runtime.uvThreadpoolSize !== 8) {
  throw new Error(
    `uv-threadpool-no-override test failed: expected derived.runtime.uvThreadpoolSize=8, got ${runtime.uvThreadpoolSize}`
  );
}
if (runtime.effectiveUvThreadpoolSize !== 4) {
  throw new Error(
    `uv-threadpool-no-override test failed: expected derived.runtime.effectiveUvThreadpoolSize=4, got ${runtime.effectiveUvThreadpoolSize}`
  );
}
