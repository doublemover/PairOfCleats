...
const languageFiles = fs.readdirSync(languageDocDir)
  .filter((fileName) => fileName.endsWith('.md'))
  .filter((fileName) => fileName !== 'README.md' && fileName !== 'TEMPLATE.md');

assert.equal(languageFiles.length > 0, true, 'language contract directory must include language spec files');

for (const fileName of languageFiles) {
  const filePath = path.join(languageDocDir, fileName);
...
console.log('usr language contract freshness and ownership rotation checks passed');
