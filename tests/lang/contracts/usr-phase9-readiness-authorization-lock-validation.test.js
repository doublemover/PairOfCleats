...
for (const testId of [
  ...
  assert.equal(ciOrderText.includes(testId), true, `ci order missing phase-9 readiness authorization lock test: ${testId}`);
  assert.equal(ciLiteOrderText.includes(testId), true, `ci-lite order missing phase-9 readiness authorization lock test: ${testId}`);
}
