...
for (const fragment of [
  ...
  assert.equal(rolloutSpecText.includes(fragment), true, `rollout migration contract missing conformance exit lock fragment: ${fragment}`);
}

assert.equal(roadmapText.includes('### N.22 Phase 11-13 conformance exit-integrity lock'), true, 'roadmap must include Appendix N.22 conformance exit lock policy');
...
assertTestsPresent(
  ...
  'conformance exit lock umbrella',
  ciOrderText,
  ciLiteOrderText
);

console.log('usr conformance exit lock validation checks passed');
