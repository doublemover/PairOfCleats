...
if (missingScripts.length) {
  console.error(`Workflow references missing scripts: ${missingScripts.join(', ')}`);
  process.exit(1);
}

const nodeVersionRegex = /node-version:\s*['"]?24\.13\.0['"]?/;
if (!nodeVersionRegex.test(workflowText)) {
  console.error('Workflow does not pin Node 24.13.0');
  process.exit(1);
}

console.log('workflow contract test passed');
