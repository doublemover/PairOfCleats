...
const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'poc-build-state-log-repair-'));
...
try {
  ...
  assert.match(eventsText, /"type":"checkpoint"/, 'expected identical patch replay to restore missing events log');
  assert.match(deltasText, /"op":"snapshot"/, 'expected identical patch replay to restore missing delta log snapshot');
  assert.match(deltasText, /"path":"\/currentPhase"/, 'expected identical patch replay to restore delta entries');

  console.log('build state log repair test passed');
} finally {
