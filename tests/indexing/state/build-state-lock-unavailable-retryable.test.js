...
try {
  await assert.rejects(
    ...
    (error) => {
      ...
      assert.equal(error?.buildState?.retryable, true);
      assert.equal(error?.buildState?.reason, 'lock-unavailable');
      return true;
    },
    'expected best-effort applyStatePatch to surface retryable lock-unavailable error'
  );
} finally {
