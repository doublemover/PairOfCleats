...
const testRoot = path.join(root, '.testCache', 'tree-sitter-scheduler-native-language-contract');
...
`
  },
  {
    ...
    expectedNames: ['Widget', 'Widget.run', 'makeWidget'],
...
for (const fixture of fixtures) {
  const abs = path.join(fixtureDir, fixture.file);
  fixture.abs = abs;
  fixture.containerPath = toPosix(path.relative(root, abs));
  await fs.writeFile(abs, fixture.content, 'utf8');
}
...
console.log(`tree-sitter scheduler native language contract ok (${fixtures.length} languages)`);
