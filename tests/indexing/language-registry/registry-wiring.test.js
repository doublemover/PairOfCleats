...
assert.equal(getLanguageForFile('.jsonc', 'config/deno.jsonc')?.id, 'json');
assert.equal(getLanguageForFile('.resolved', 'swift/Package.resolved')?.id, 'json');
assert.equal(getLanguageForFile('.toml', 'config/app.toml')?.id, 'toml');
assert.equal(getLanguageForFile('', 'python/Pipfile')?.id, 'toml');
...
assert.equal(getLanguageForFile('', 'go.mod')?.id, 'go');
