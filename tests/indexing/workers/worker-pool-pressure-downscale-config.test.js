...
assert.equal(defaults.downscaleRssThreshold, 0.95, 'expected default RSS downscale threshold');
assert.equal(defaults.downscaleGcThreshold, 0.92, 'expected default GC downscale threshold');
assert.equal(defaults.downscaleMinWorkers, 5, 'expected default minimum workers to track half of maxWorkers');
assert.equal(defaults.memoryPressure.watermarkSoft, 0.985, 'expected default soft pressure watermark');
assert.equal(defaults.memoryPressure.watermarkHard, 0.995, 'expected default hard pressure watermark');
