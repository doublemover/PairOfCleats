...
if (pythonPolicy.status !== 0) {
  console.error('sublime-map-behavior: required python toolchain is missing');
...
const script = path.join(root, 'tests', 'helpers', 'sublime', 'map_behavior.py');
const result = spawnSync(python, [script], { encoding: 'utf8' });

if (result.status !== 0) {
  console.error('sublime-map-behavior: python behavior test failed');
  if (result.stdout) console.error(result.stdout);
  if (result.stderr) console.error(result.stderr);
  process.exit(result.status || 1);
}

console.log('sublime map behavior test passed');
