...
if (pythonPolicy.status !== 0) {
  console.error('sublime-navigation-behavior: required python toolchain is missing');
...
const script = path.join(root, 'tests', 'helpers', 'sublime', 'navigation_behavior.py');
const result = spawnSync(python, [script], { encoding: 'utf8' });

if (result.status !== 0) {
  console.error('sublime-navigation-behavior: python behavior test failed');
  if (result.stdout) console.error(result.stdout);
  if (result.stderr) console.error(result.stderr);
  process.exit(result.status || 1);
}

console.log('sublime navigation behavior test passed');
