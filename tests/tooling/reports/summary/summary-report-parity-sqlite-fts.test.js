...
if (result.status !== 0) {
  console.error('summary report parity (sqlite-fts) failed: script error.');
  if (result.stderr) console.error(result.stderr.trim());
  process.exit(result.status ?? 1);
}

if (!fs.existsSync(outPath)) {
  console.error('summary report parity (sqlite-fts) failed: output JSON missing.');
  process.exit(1);
}

const payload = JSON.parse(fs.readFileSync(outPath, 'utf8'));
if (!payload.summary || !payload.results) {
  console.error('summary report parity (sqlite-fts) failed: missing summary fields.');
  process.exit(1);
}
