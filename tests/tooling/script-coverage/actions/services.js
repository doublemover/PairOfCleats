...
export const buildServiceActions = ({ root, runNode }) => [
  ...
  },
  {
    ...
    run: () => runNode('mcp-schema-test', path.join(root, 'tests', 'services', 'mcp', 'mcp-schema.test.js')),
    covers: ['mcp-schema-test']
  },
  {
    label: 'mcp-robustness-test',
    run: () => runNode('mcp-robustness-test', path.join(root, 'tests', 'services', 'mcp', 'mcp-robustness.test.js')),
  ...
  },
  {
    ...
    run: () => runNode('api-server-stream-test', path.join(root, 'tests', 'services', 'api', 'api-server-stream.test.js')),
    covers: ['api-server-stream-test']
  },
  {
    label: 'indexer-service-test',
    run: () => runNode('indexer-service-test', path.join(root, 'tests', 'services', 'indexer', 'indexer-service.test.js')),
    covers: ['indexer-service-test']
  },
  {
    label: 'service-queue-test',
    run: () => runNode('service-queue-test', path.join(root, 'tests', 'services', 'queue', 'service-queue.test.js')),
