...
assert.equal(
  ...
  true,
  'expected rust-analyzer to block when Cargo workspace markers are absent near selected docs'
);
assert.equal((result.byChunkUid instanceof Map ? result.byChunkUid.size : Object.keys(result.byChunkUid || {}).length), 0, 'expected rust-analyzer provider to skip output when no workspace applies');
