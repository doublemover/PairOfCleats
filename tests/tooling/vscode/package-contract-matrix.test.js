...
for (const [commandId, title] of expectedCommands.entries()) {
  ...
  if (!command || command.title !== title) {
    console.error(`VS Code extension command drifted for ${commandId}.`);
    process.exit(1);
  }
}
...
const requireMenuEntries = (entries, required, label) => {
  for (const [commandId, expectedWhen] of required.entries()) {
    const menu = entries.find((entry) => entry.command === commandId);
    if (!menu || menu.when !== expectedWhen) {
      console.error(`VS Code ${label} drifted for ${commandId}.`);
      process.exit(1);
    }
  }
};

requireMenuEntries(commandPaletteMenus, new Map([
...
requireMenuEntries(editorContextMenus, new Map([
  ['pairofcleats.searchSelection', 'editorTextFocus && editorHasSelection'],
  ['pairofcleats.searchSymbolUnderCursor', 'editorTextFocus'],
...
requireMenuEntries(viewTitleMenus, new Map([
...
requireMenuEntries(viewItemContextMenus, new Map([
...
for (const [commandId, expectedWhen] of new Map([
  ['pairofcleats.searchSelection', 'editorTextFocus && editorHasSelection'],
  ['pairofcleats.searchSymbolUnderCursor', 'editorTextFocus'],
  ['pairofcleats.repeatLastSearch', 'workbenchState != empty']
]).entries()) {
  const binding = keybindings.find((entry) => entry.command === commandId);
  if (!binding || binding.when !== expectedWhen) {
    console.error(`VS Code keybinding drifted for ${commandId}.`);
    process.exit(1);
  }
}
...
for (const markdownPath of ['walkthroughs/first-search.md', 'walkthroughs/operations.md']) {
  if (!fs.existsSync(path.join(root, 'extensions', 'vscode', markdownPath))) {
...
const expectedSettings = Object.entries(keyMap).map(([contractKey, prefix]) => `${prefix}.${settings[contractKey]}`).filter(Boolean);
...
const guideSettings = new Set(Array.from(guide.matchAll(/- `([^`]+)`/g), (match) => match[1]).filter((entry) => entry.startsWith('pairofcleats.')));
for (const prop of expectedSettings) {
...
if (configProps['pairofcleats.apiTimeoutMs']?.type !== 'number' || configProps['pairofcleats.apiTimeoutMs']?.minimum !== 1) {
  console.error('VS Code apiTimeoutMs setting contract drifted.');
  process.exit(1);
}
...
const packagedManifestPath = `${archivePath}.manifest.json`;
if (!fs.existsSync(archivePath) || !fs.existsSync(packagedManifestPath)) {
  console.error('package-contract-matrix failed: archive outputs missing');
  process.exit(1);
}
const packagedManifest = JSON.parse(fs.readFileSync(packagedManifestPath, 'utf8'));
const paths = Array.isArray(packagedManifest.entries) ? packagedManifest.entries.map((entry) => entry.path) : [];
for (const entry of [
  ...
  if (!paths.includes(entry)) {
    console.error(`package-contract-matrix failed: missing shipped entry ${entry}`);
    process.exit(1);
  }
}

console.log('vscode package contract matrix test passed');
