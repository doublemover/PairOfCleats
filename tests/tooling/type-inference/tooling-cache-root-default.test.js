...
const posixToolingDir = __resolveDefaultToolingCacheDirForTests({
  rootDir: posixRoot,
  buildRoot: posixBuildRoot
});
assert.equal(
  posixToolingDir,
  path.posix.resolve('/cache-root/repos/project/tooling-cache'),
  'expected POSIX build-root cache layout to resolve tooling cache under stable repo cache root'
);
