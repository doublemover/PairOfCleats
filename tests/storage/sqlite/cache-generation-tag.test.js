...
cache.set(dbPath, dbA, {
  generationTag: { mode: 'code', buildId: 'build-a' }
});

assert.equal(
  cache.get(dbPath, { generationTag: { mode: 'code', buildId: 'build-a' } }),
  dbA,
  'expected matching generation tag to hit sqlite cache'
);
assert.equal(
  cache.get(dbPath, { generationTag: { mode: 'code', buildId: 'build-b' } }),
  null,
  'expected generation tag mismatch to miss sqlite cache'
);

cache.set(dbPath, dbB, {
  generationTag: { mode: 'code', buildId: 'build-b' }
});
...
cache.close(dbPath, { generationTag: { mode: 'code', buildId: 'build-b' } });
