...
const runReplaceWithRenameFault = async ({
  tempPath,
  finalPath,
  shouldFailRename,
  failureMessage
  ...
  fsPromises.rename = async (from, to) => {
    const code = shouldFailRename(from, to);
    if (code) {
...
const runCrossDeviceFallbackCase = async () => {
  ...
  const failed = await runReplaceWithRenameFault({
    tempPath,
    finalPath,
    shouldFailRename: (from, to) => (from === tempPath && to === finalPath ? 'EXDEV' : null),
    failureMessage: 'expected sqlite replace to fail closed on EXDEV'
  });
...
const runRestoreBackupCase = async () => {
  ...
  await runReplaceWithRenameFault({
    tempPath,
    finalPath,
    shouldFailRename: (from, to) => (from === tempPath && to === finalPath ? 'ENOENT' : null),
    failureMessage: 'expected sqlite replace to fail when temp promote cannot complete'
  });
...
const runCrossDeviceBackupMoveCase = async () => {
  ...
  const failed = await runReplaceWithRenameFault({
    tempPath,
    finalPath,
    shouldFailRename: (from, to) => (from === finalPath && to === backupPath ? 'EXDEV' : null),
    failureMessage: 'expected sqlite replace to fail when final->backup crosses devices'
  });
  assert.equal(failed?.code, 'ERR_SQLITE_REPLACE_CROSS_DEVICE');
  assert.equal(await fsPromises.readFile(finalPath, 'utf8'), 'before');
  assert.equal(await fsPromises.readFile(tempPath, 'utf8'), 'after');
