...
const payload = {
  ...
  types: {
    facts: [{ role: 'return', type: 'Promise<string>' }],
    evidence: {
      included: true,
      state: 'partial',
      count: 1,
  ...
  risk: {
    ...
    summary: {
      ...
      ruleRoles: { sources: 1, sinks: 1, sanitizers: 0 },
...
const cases = [
  {
    name: 'markdown render includes pack evidence, truncation, warnings, and risk sections',
    run() {
  ...
  {
    name: 'JSON render preserves pack evidence and risk detail contracts',
    run() {
  ...
  {
    name: 'SARIF export carries provenance, pack truncation, and partial-flow metadata',
    run() {
      const jsonPayload = renderCompositeContextPackJson(payload);
      assert.ok(jsonPayload.rendered.sarif);
      ...
      assert.equal(
        jsonPayload.rendered.sarif.runs[0].results[0].codeFlows[0].threadFlows[0].locations[0].properties.pairOfCleats.watchWindow.calleeNormalized,
        'query'
      );
      assert.deepEqual(
      ...
      assert.equal(jsonPayload.rendered.sarif.runs[0].properties.pairOfCleats.partialFlowSelection.totalPartialFlows, 5);
...
console.log('composite context pack contract matrix test passed');
